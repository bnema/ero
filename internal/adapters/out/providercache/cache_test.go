...
func TestCacheRoundTripNormalizedSnapshot(t *testing.T) {
	...
	if err := store.SaveProviderSnapshot(context.Background(), snapshot); err != nil { t.Fatal(err) }
	got, ok, err := store.LoadProviderSnapshot(context.Background(), key)
	if err != nil { t.Fatal(err) }
	if !ok { t.Fatal("expected cached snapshot") }
...
func TestPreferenceRoundTrip(t *testing.T) {
	...
	if _, ok, err := store.LoadActiveProviderKey(context.Background(), repoID); err != nil || ok { t.Fatalf("empty load = ok %v err %v", ok, err) }
	if err := store.SaveActiveProviderKey(context.Background(), repoID, "provider-key"); err != nil { t.Fatal(err) }
	got, ok, err := store.LoadActiveProviderKey(context.Background(), repoID)
	if err != nil { t.Fatal(err) }
	if !ok || got != "provider-key" { t.Fatalf("got %q ok %v", got, ok) }
}
