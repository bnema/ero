...
func NewProviderRuntimeCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:    "__provider",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch providerID {
			case "codex":
				return runCodexRuntime(cmd.InOrStdin(), cmd.OutOrStdout())
			default:
				return fmt.Errorf("unknown builtin provider: %s", providerID)
			}
		},
	}
	return cmd
...
// runCodexRuntime implements the JSON-lines plugin protocol for the bundled
// Codex review provider. Each request is dispatched with a bounded context
// derived from the runtime's background context.
func runCodexRuntime(in io.Reader, out io.Writer) error {
	...
	for scanner.Scan() {
		...
		// Each request gets a context derived from the runtime context so
		// the lifetime of any publish operation is bounded.
		resp := dispatchCodexRequest(runtimeCtx, req)
		resp.ID = req.ID
